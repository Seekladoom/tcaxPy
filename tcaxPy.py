...
# Please visit tcax.org to get the latest information
...
# Revision: ver0.8.0.1 build20121228
# Revision: ver0.8.0.2 build20181209
# Revision: ver0.8.0.3 build20210910
...
def getTXTNUM(i):
    return __tcax_data[29][i]

def getBT(i):
    return __tcax_data[27][i]

def getET(i):
    return __tcax_data[28][i]

def getSK(i, j):
    return __tcax_data[31][i][j]

def getKT(i, j):
    return __tcax_data[30][i][j]

def getTXT(i, j):
    return __tcax_data[32][i][j]
...
def getposX(i, j, an = None, offsetX = None, offsetY = None):
    POS = getpos(i, j, an, offsetX, offsetY)
    return POS[0]

def getposY(i, j, an = None, offsetX = None, offsetY = None):
    POS = getpos(i, j, an, offsetX, offsetY)
    return POS[1]

def getpos_vert(i, j, an = None, offsetX = None, offsetY = None):
    if an == None:
        an = int(__tcax_data[8])
    if offsetX == None:
        offsetX = __tcax_data[9]
    if offsetY == None:
        offsetY = __tcax_data[10]
    if an == 1:
    	initPosX =  offsetX + int(__tcax_data[4]/2+0.5)
    	initPosY = __tcax_data[6] - __tcax_data[49][i]- offsetY
    elif an == 2:
    	initPosX = int(__tcax_data[5] / 2 + 0.5) + offsetX + int(__tcax_data[4]/2+0.5)
    	initPosY = __tcax_data[6] - __tcax_data[49][i]- offsetY
    elif an == 3:
    	initPosX = __tcax_data[5] - int(__tcax_data[4] / 2 + 0.5) - offsetX
    	initPosY = __tcax_data[6] - __tcax_data[49][i]- offsetY
    elif an == 4:
    	initPosX =  offsetX + int(__tcax_data[4] / 2+0.5)
    	initPosY = int((__tcax_data[6] - __tcax_data[49][i]) / 2+0.5) - offsetY
    elif an == 5:
    	initPosX = int(__tcax_data[5] / 2 + 0.5) + offsetX + int(__tcax_data[4]/2+0.5)
    	initPosY = int((__tcax_data[6] - __tcax_data[49][i]) / 2 + 0.5) - offsetY
    elif an == 6:
    ...
    posX = initPosX 
    posY = initPosY + __tcax_data[48][i][j] + int(__tcax_data[47][i][j]/2+0.5)

    return (posX,posY)  

def getposX_vert(i, j, an = None, offsetX = None, offsetY = None):
    POS = getpos_vert(i,j,an,offsetX,offsetY)
    return POS[0]

def getposY_vert(i, j, an = None, offsetX = None, offsetY = None):
    POS = getpos_vert(i, j, an, offsetX, offsetY)
    return POS[1]
...
def UseSitePackages():      # invoke this function right after importing tcaxPy module if you want to use Python 3rd party packages
    ...
    sys.path.append(sys.prefix + '\\Lib\\util')
    sys.path.append(sys.prefix + '\\Lib\\site-packages')

########################################## Main FX Function #################################################

def SubL(Start = 0, End = 0, Layer = 0, Style = 'TCMS'):
    return 'Dialogue: {0},{1},{2},{3},,0,0,0,,'.format(int(Layer), FmtTime(Start), FmtTime(End), Style)
...
def fsc1(x):      # 生成mod标签 \fsc
    if x == int(x):
        X = str(int(x))
    else:
        X = str(format(x, '.2f'))
    return '\\fsc{0}'.format(int(X))

def fe(a):
    return '\\fe{0}'.format(int(a))
...
def yblur(x):
    if x == int(x):
        ...
        X = str(format(x, '.2f'))
    return '\\yblur{0}'.format(X)

def p(a):
    return '\\p{0}'.format(int(a))
    
def pbo(a):
    return '\\pbo-{0}'.format(int(a))    # 请配合p使用，pbo-标签是用来调整绘图代码基线位置的

def r(Style):
    return '\\r{0}'.format(Style)

def b(a):
    return '\\b{0}'.format(int(a))

def i(a):
    return '\\i{0}'.format(int(a))

def s(a):
    return '\\s{0}'.format(int(a))

def u(a):
    return '\\u{0}'.format(int(a))
...
def blend(mode):    # 目前支持的模式有：over、add、sub、mult、scr、diff
    return '\\blend{0}'.format(mode)

def ortho(a):    # 必须配合至少一个frx、fry、frz标签使用
    return '\\ortho{0}'.format(a)
...
def org(x, y):
    return '\\org({0},{1})'.format(int(x), int(y))
...
def color(a):                                    # note that c is a RGB string
    return '\\c&H{0}&'.format(a)

def color1(a):
    return '\\1c&H{0}&'.format(a)

def color2(a):
    return '\\2c&H{0}&'.format(a)

def color3(a):
    return '\\3c&H{0}&'.format(a)

def color4(a):
    return '\\4c&H{0}&'.format(a)
...
def rnds(a):    # 必须配合至少一个其他的rnd标签使用，rnds对其他rnd标签的直接影响才会体现出来
    if a == int(a):
        A = str(int(a))
    else:
        A = str(format(a, '.2f'))
    return '\\rnds{0}'.format(A)

def img1(path, x = None, y = None, PathType = None):
...
def movevc(x1, y1, x2, y2, t1 = None, t2 = None):
    if x1 == int(x1) and y1 == int(y1) and x2 == int(x2) and y2 == int(y2):
        ...
        Y2 = str(format(y2, '.2f'))
    if t2 == None:
        return '\\movevc({0},{1},{2},{3})'.format(X1, Y1, X2, Y2)
    else:
        return '\\movevc({0},{1},{2},{3},{4},{5})'.format(X1, Y1, X2, Y2, int(t1), int(t2))

def jitter(left, right, top, bottom, period = None, seed = None):
    if left == int(left) and right == int(right) and top == int(top) and bottom == int(bottom):
        L = str(int(left))
        R = str(int(right))
        T = str(int(top))
        B = str(int(bottom))
    ...
    if period == None:    # 周期
        return '\\jitter({0},{1},{2},{3})'.format(L, R, T, B)
